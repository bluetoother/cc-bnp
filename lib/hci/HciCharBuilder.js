...
ValObj.factory = function (uuid) {
    ...
    if (!charValsMeta) { throw new Error(uuid + " doesn't exist"); }
...
ValObj.prototype.transToValObj = function (argInst) {
    var cmdParams = this.getCharVals().params,
        cmdExtra = this.getCharVals().extra,
        self = this;
    ...
    if (cmdExtra) {
        var cmdExtraParams = this.getCharVals().extra.params;
        for (var i = 0; i < cmdExtraParams.length; i++) {
            if ((argInst.Flags & cmdExtra.flags[i]) === cmdExtra.result[i]) {
...
ValObj.prototype.getHciCharBuf = function () {
    var self = this,
        dataBuf = Concentrate(),
...
function builder(dataBuf, paramVal, paramType) {
    ...
    switch (paramType) {
        ...
        case 'string':
            ...
            break;
        case 'addr3':
            var tmpBuf = (new Buffer(3)).fill(0);

            paramVal = addr2Buf(paramVal, 3);
            for (var j = 0; j < 3; j++) {
                tmpBuf.writeUInt8(paramVal.readUInt8(j), (2-j));
            }
            dataBuf = dataBuf.buffer(tmpBuf);
            break;
        case 'addr5':
            var tmpBuf = (new Buffer(5)).fill(0);

            paramVal = addr2Buf(paramVal, 5);
            for (var j = 0; j < 5; j++) {
                tmpBuf.writeUInt8(paramVal.readUInt8(j), (4-j));
            }
            dataBuf = dataBuf.buffer(tmpBuf);
            break;
        case 'addr6':
...
function addr2Buf (addr, len) {
    var addrArr = [],
        chunk;

    if (_.startsWith(addr, '0x')) { addr = addr.slice(2); }
    for (var i = 0; i < len; i += 1) {
        chunk = addr.substring(0, 2);
        addrArr.push(_.parseInt(chunk, 16));
        addr = addr.slice(2);
    }

    return new Buffer(addrArr);
}
