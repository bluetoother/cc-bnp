...
ValObj.prototype.transToValObj = function (argInst) {
    ...
    if (_.isObject(this.uuid)) {
        for (var i = 0; i < _.values(argInst).length; i += 1) {
            ...
            if (charVals.extra) {
                _.forEach(charVals.extra.params, function (param, idx) {
                    if ((argInst[_.keys(argInst)[i]].Flags & charVals.extra.flags[idx]) === charVals.extra.result[idx]) {
                        if (argInst[_.keys(argInst)[i]][param]) self['handle' + i][param] = argInst[_.keys(argInst)[i]][param];
        ...
        if (charVals.extra) {
            _.forEach(charVals.extra.params, function (param, idx) {
                if ((argInst.Flags & charVals.extra.flags[idx]) === charVals.extra.result[idx]) {
                    if (argInst[param]) self[param] = argInst[param];
                    else throw new Error('The argument object has incorrect properties.');
                }
            });
        }
...
ValObj.prototype.getHciCharBuf = function (callback) {
    ...
    if (_.isObject(this.uuid)) {
        for (var i = 0; i < _.size(this.uuid); i += 1) {
            ...
            if (charVals.extra) {
                _.forEach(charVals.extra.params, function (param, idx) {
                    if ((self['handle' + i].Flags & charVals.extra.flags[idx]) === charVals.extra.result[idx]) {
                        builder(dataBuf, self['handle' + i][param], charVals.extra.types[idx]);
                    }
                });
            }
        }
        ...
        if (charVals.extra) {
            _.forEach(charVals.extra.params, function (param, idx) {
                if ((self.Flags & charVals.extra.flags[idx]) === charVals.extra.result[idx]) {
                    builder(dataBuf, self[param], charVals.extra.types[idx]);
                }
            });
        }       
    }
