    ...
    return function (msg) {
        ...
        if (_.startsWith(msg.evtName, 'Hci')) {
            if (msg.data.status !== 0) {
                var errMsg = 'Error message: ' +BHCI.HciErrCode.get(msg.data.status).key;
...
function genCmdStatusEvtHdlr(deferred) {
    return function (msg) {
        ...
        if (msg.data.status !== 0) {
            var errMsg = 'Error message: ' +BHCI.GenericStatus.get(msg.data.status).key;
    ...
    switch (subGroup) {
        ...
        case 'Gatt':
            ...
            self.attErrHdlr = function (msg) {
                var errMsg = 'Error message: ' + BHCI.AttErrCode.get(msg.errCode).key;
