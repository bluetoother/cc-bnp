...
function appLevelHandler(msg) {
    var deferred = Q.defer(),
        subGroup = BHCI.EvtSubGroup.get(msg.subGroup).key,
    ...
    argObj.getHciEvtPacket(msg.len, msg.data).then(function (result) {
        ...
        if (evtApiName === 'GapCmdStatus') {
            ...
            deferred.resolve(objToEmit);
        } else if (evtApiName === 'AttHandleValueNoti' || evtApiName === 'AttHandleValueInd') {
            if (bleHci._uuidHdlTable[parsed.data.connHandle]) {
                uuidInTable = bleHci._uuidHdlTable[parsed.data.connHandle][parsed.data.handle];
            }

            if (uuidInTable) {
                evtChar = charDiscrim(shrinkUuid(uuidInTable));

                evtChar.getCharValPacket(parsed.data.value).then(function (result) {
                ...
                bleHci.execCmd('Gatt', 'DiscAllChars', {connHandle: parsed.data.connHandle, startHandle: parsed.data.handle - 1, endHandle: parsed.data.handle}).then(function (result) {
                    evtChar = charDiscrim(shrinkUuid(result[1].AttReadByTypeRsp0.data.attrVal0.uuid));

                    return evtChar.getCharValPacket(parsed.data.value);
                }).then(function (result) {
                    parsed.data.value = result;
                    deferred.resolve(objToEmit);
                }).fail(function (err) {
        ...
        } else if (evtApiName === 'AttFindByTypeValueReq') {
            evtChar = charDiscrim(shrinkUuid(parsed.data.type));
        ...
        } else if (evtApiName === 'AttWriteReq') {
            if (bleHci._uuidHdlTable['65534']) {
                uuidInTable = bleHci._uuidHdlTable['65534'][parsed.data.handle];
            }
...
CmdStore.prototype.buildCmdChar = function (callback) {
    ...
    if (_.indexOf(BHCI.cmdValBuild, this.cmdName) > -1) {
        if (Buffer.isBuffer(this.args.value)) {
            deferred.resolve();
        } else {
            uuidHdlTable = bleHci._uuidHdlTable[this.args.connHandle];
            ...
            switch (this.cmdName) {
                ...
                case 'GattWriteCharDesc':
                    if (!this.uuid && uuidHdlTable) { this.uuid = uuidHdlTable[this.args.handle]; }

                    if (this.uuid && hciCharMeta[this.uuid]) {
                        try {
                            cmdchar = charBuild(shrinkUuid(this.uuid)).transToValObj(this.args.value).getHciCharBuf();
                            this.args.value = cmdchar;
                            deferred.resolve();
                        } catch (err) {
                            deferred.reject(err);
                        }
                        ...
                        bleHci.execCmd('Gatt', 'DiscAllChars', {connHandle: this.args.connHandle, startHandle: this.args.handle - 1, endHandle: this.args.handle}).then(function (result) {
                            var uuid = result[1].AttReadByTypeRsp0.data.attrVal0.uuid;
                            ...
                            if (hciCharMeta[uuid]) {
                                ...
                                deferred.reject(new Error('Args do not have uuid or the uuid not be registered.'));
                            }
                        }).fail(function (err) {
                            deferred.reject(err);
                        }).done;
                ...
                case 'AttFindByTypeValueReq':
                    if (this.args.type && hciCharMeta[this.args.type]) {
                        try {
                            cmdchar = charBuild(shrinkUuid(this.args.type)).transToValObj(this.args.value).getHciCharBuf();
                            this.args.value = cmdchar;
                            deferred.resolve();
                        } catch (err) {
                            deferred.reject(err);
                        ...
                        deferred.reject(new Error('Type fields must be filled.'));
                    }
                    break;
                case 'AttReadByTypeRsp': 
                    if (this.uuid && hciCharMeta[this.uuid]) {
                        for (var i = 0; i < (_.keys(this.args.data).length / 2); i += 1) {
                            try {
                                cmdchar = charBuild(shrinkUuid(this.uuid)).transToValObj(this.args.data['attrVal' + i]).getHciCharBuf();
                                this.args.data['attrVal' + i] = cmdchar;
                            } catch (err) {
                                errMsg = err;
                            }
                        }
...
CmdStore.prototype.buildEvtChar = function (evtObj, callback) {
    var self = this,
        ...
        charObj,
        charToResolve = [],
        uuidHdlTable = bleHci._uuidHdlTable;

    if (_.keys(this.evtHandlersTable)[1] === 'AttReadRsp') {
        if (!this.uuid && uuidHdlTable[this.args.connHandle]) {
            this.uuid = uuidHdlTable[this.args.connHandle][this.args.handle];
        }

        if (this.uuid) {
            charDiscrim(shrinkUuid(this.uuid)).getCharValPacket(evtObj[1].AttReadRsp.value).then(function (result) {
        ...
        uuid = this.args.type;
        evtChar = charDiscrim(shrinkUuid(uuid));
            ...
            _.forEach(evtObj[1], function (charObj) {
                if (charObj.status === 0) {
                    ...
                    for(var i = 0; i < (_.keys(charObj.data).length / 2); i += 1) {
                        ...
                        uuidHdlTable[charObj.connHandle][charObj.data['attrVal' + i].hdl] = uuid; 
                    }
                }
            });
            deferred.resolve(evtObj);
            ...
            _.forEach(evtObj[1], function (charObj) {
                if (charObj.status === 0) {
                    ...
                    _.forEach(_.keys(charObj.data), function (name) {
                        if (_.startsWith(name, 'attrHandle')) { 
...
bleHci.on('GapLinkTerminated', function (result) {
    if (result.data.status === 0) {
        delete this._uuidHdlTable[result.data.connHandle];
    }
});
