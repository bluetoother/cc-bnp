...
function appLevelHandler(msg) {
    ...
    argObj.getHciEvtPacket(msg.len, msg.data).then(function (result) {
        ...
        if (evtApiName === 'GapCmdStatus') {
            ...
            var uuidInTable;
            if (bleHci._uuidHdlTable[objToEmit.data.data.connHandle]) {
                uuidInTable = bleHci._uuidHdlTable[objToEmit.data.data.connHandle][objToEmit.data.data.handle];
            }

            if (uuidInTable) {
                var evtChar = charDiscrim(uuidInTable);

                evtChar.getCharValPacket(objToEmit.data.data.value).then(function (result) {
                    objToEmit.data.data.value = result;
                    deferred.resolve(objToEmit);
                }).fail(function (err) {
                    deferred.reject(err);
                });
            } else {
                bleHci.execCmd('Gatt', 'DiscAllChars', {connHandle: objToEmit.data.data.connHandle, startHandle: objToEmit.data.data.handle - 1, endHandle: objToEmit.data.data.handle}).then(function (result) {
                    var uuid = result[1].AttReadByTypeRsp0.data.attrVal0.uuid,
                        evtChar = charDiscrim(uuid);

                    return evtChar.getCharValPacket(objToEmit.data.data.value);
                }).then(function (result) {
                    objToEmit.data.data.value = result;
                    deferred.resolve(objToEmit);
                }).fail(function (err) {
        ...
        if (Buffer.isBuffer(self.args.value)) {
            ...
            switch (this.cmdName) {
                ...
                case 'GattWriteCharDesc':
                    if (!this.uuid && bleHci._uuidHdlTable[this.args.connHandle]) {
                        this.uuid = bleHci._uuidHdlTable[this.args.connHandle][this.args.handle];
                    }
    ...
    if (_.keys(this.evtHandlersTable)[1] === 'AttReadRsp') {
        ...
        if (this.cmdName === 'GattDiscAllChars') {
            _.forEach(evtObj[1], function (charObj) {
                if (charObj.status === 0) {
                    if (!bleHci._uuidHdlTable[charObj.connHandle]) { 
                        bleHci._uuidHdlTable[charObj.connHandle] = {}; 
                    }
                    for(var i = 0; i < (_.keys(charObj.data).length / 2); i += 1) {
                        ...
                        bleHci._uuidHdlTable[charObj.connHandle][charObj.data['attrVal' + i].hdl] = uuid; 
                    }
                }
            });
            deferred.resolve(evtObj);
                ...
                if (charObj.status === 0) {
                    ...
                    _.forEach(_.keys(charObj.data), function (name) {
                        if (_.startsWith(name, 'attrHandle')) { 
                            bleHci._uuidHdlTable[charObj.connHandle][charObj.data[name]] = uuid; 
                        }
