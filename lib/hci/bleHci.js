...
function appLevelHandler(msg) {
    ...
    argObj.getHciEvtPacket(msg.len, msg.data).then(function (result) {
        ...
        if (evtApiName === 'GapCmdStatus') {
            ...
            if (bleHci.uuidHdlTable[parsed.data.connHandle]) {
                uuidInTable = bleHci.uuidHdlTable[parsed.data.connHandle][parsed.data.handle];
            }
            ...
            if (bleHci.uuidHdlTable['65534']) {
                uuidInTable = bleHci.uuidHdlTable['65534'][parsed.data.handle];
            }
