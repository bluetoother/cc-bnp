...
BleHci.prototype._addListenerAndInvokeCmd = function (subGroup, cmd) {
    ...
    cmdSto.buildCmdChar().then(function () {
        ...
        // transResultFormat(subGroup, cmdName, result);
        ...
        switch (apiName) {
            ...
            case 'AttHandleValueInd':
                if (bleHci.uuidHdlTable[data.connHandle] && bleHci.uuidHdlTable[data.connHandle][data.handle]) {
                    ...
                    .then(function (result) {
                        deferred.resolve(result[1].AttReadByTypeRsp0.data.attrVal0.uuid);
                    }).fail(function (err) {
                        deferred.reject(err);
                    }).done();
                }
...
function transResultFormat (subGroup, cmdName, result) {
    var newResult;

    if (_.isEmpty(result)) {
        return null;
    } else {
        newResult = _.values(result.shift())[0];
        newResult.collector = {};
