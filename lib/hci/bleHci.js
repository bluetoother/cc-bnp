            ...
            switch (this.cmdName) {
                ...
                case 'AttReadMultiRsp':
                    if (this.uuid) {
                        ...
                        for (var i = 0; i < (_.keys(self.args.value).length); i += 1) {
                            var temp = charBuild(this.uuid['uuid' + i]).transToValObj(_.values(self.args.value)[i]).getHciCharBuf();
                            cmdCharArr.push(temp);
                            totalLength += temp.length;
                        }

                        self.args.value = Buffer.concat(cmdCharArr, totalLength);
                        deferred.resolve();
                    } else {
                        deferred.reject(new Error('The value are not Buffer.'));
                    }
...
CmdStore.prototype.buildEvtChar = function (evtObj, callback) {
    ...
    if (_.keys(this.evtHandlersTable)[1] === 'AttReadRsp') {
        if (this.uuid) {
            ...
            bleHci.execCmd('Gatt', 'DiscAllChars', {connHandle: this.args.connHandle, startHandle: this.args.handle - 1, endHandle: this.args.handle}).then(function (result) {
                uuid = result[1].AttReadByTypeRsp0.data.attrVal0.uuid;
