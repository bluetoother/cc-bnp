...
CmdStore.prototype.buildHandlers = function () {
    ...
    switch (subGroup) {
        ...
        case 'Gatt':
            ...
            self.attErrHdlr = function (msg) {
                var errMsg = BHCI.AttErrCode.get(msg.errCode).key;
                self._rejectAll(new ccBnpError.AttError(errMsg, msg.errCode));
                clearTimeout(self.tmrOut);
                self.bleHci._invokeNextCmd(self.subGroup, self.cmd);                
            };

            self.bleHci.once('AttErrorRsp:' + cmdId, self.attErrHdlr);
            break;
...
CmdStore.prototype.buildEvtChar = function (evtObj, callback) {
    ...
    if (_.keys(this.evtHandlersTable)[1] === 'AttReadRsp') {
        if (this.uuid) {
            var evtChar = charDiscrim(this.uuid).getCharValPacket(evtObj[1].AttReadRsp.value).then(function (result) {
        ...
        var uuid = this.args.type,
            evtChar = charDiscrim(uuid),
            charToResolve = [],
            charObj;

        for (var i = 0; i < (_.keys(evtObj[1]).length); i += 1) {
            charObj = evtObj[1]['AttReadByTypeRsp' + i];
            if (charObj.status === 0) { 
                for (var j = 0; j < (_.keys(charObj.data).length / 2); j += 1) {
                    charToResolve.push((function () {
                        var deferred = Q.defer();
                        return evtChar.getCharValPacket(charObj.data['attrVal' + j]);
                    }()));
                }
            }
        }

        Q.all(charToResolve).done(function (result) {
            for (var i = 0; i < (_.keys(evtObj[1]).length); i += 1) {
                charObj = evtObj[1]['AttReadByTypeRsp' + i];
                if (charObj.status === 0) { 
                    for (var j = 0; j < (_.keys(charObj.data).length / 2); j += 1) {
                        charObj.data['attrVal' + j] = result[i];
                    }
                }
            }
            deferred.resolve(evtObj);
        ...
        deferred.resolve(evtObj);
    } else if (_.keys(this.evtHandlersTable)[1] === 'AttReadByGrpTypeRsp') {
